...
Load Balancer would provide L4 traffic distribution if the IEP needs it for scenarios not well served by Application Gateway. For the ASE based web env, it is optional and would usually be introduced only for specific network or protocol requirements.
...
- Confirm that an actual L4 use case exists before introducing.
