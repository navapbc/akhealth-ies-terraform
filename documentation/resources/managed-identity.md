...
Managed Identity provides Azure native workload identity so applications and automation can authenticate to Azure services without storing credentials.
...
- The IEP prefers system assigned identities for ease of management and zero-trust architecture. Decide where system assigned identities are sufficient and where user assigned identities improve reuse or lifecycle control. 
- Standardize role assignment configs so access is understandable and reviewable.
- Plan how identities and role bindings will be recreated or mirrored in West Central US DR resources.
- Ensure applications use managed identity consistently throughout for Key Vault, Storage, Service Bus, etc...
- Avoid creating overly broad shared identities that weaken security posture/logical separation.
