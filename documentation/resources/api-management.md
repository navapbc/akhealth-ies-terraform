...
API Management provides the managed API front for internal or external consumers. In this environment, it can centralize API publishing, policy enforcement, versioning, and access control in front of ASE-hosted web apps and function apps.

## Region considerations

- Region selection: matters strongly because API Management is a regional service with region- and SKU-specific networking behaviors. West US 2 is the primary region, and West Central US should be considered as the secondary/DR deployment target where needed.
- Availability zones: are relevant only for supported SKUs and regions, so zonal resiliency must be verified.
...
- Align networking configs with the private first design and the app service environment based hosting model.
